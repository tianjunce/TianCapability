...
XINHUA_ARTICLE_RE = re.compile(r"^https?://(?:www\.)?news\.cn/(?!search)(?:[^?#]+)")


class XinhuaParser(BaseNewsParser):
    site_name = "新华网"
    ...
    def build_search_urls(self, query: str) -> list[str]:
        return [f"https://so.news.cn/#search/0/{quote(query)}/1/0"]

    def is_article_url(self, url: str) -> bool:
        return bool(XINHUA_ARTICLE_RE.match(url))
