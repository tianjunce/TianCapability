...
- 给系统“选不选这个 skill”提供语义依据
- 告诉 prepare 阶段应该收集什么参数
- 告诉系统哪些请求应该进入 skill，哪些不该进入
...
2. 再写 `SKILL.md`：系统什么时候该选它，应该准备哪些参数
...
如果 skill 能提供“天气事实”，而上层还能继续基于事实生成穿衣建议，那么这类问题依然应该进入 `get_weather`。
...
如果这一点不写清，系统会默认它“全支持”或“全不支持”，两种都容易错。

### 3.6 写清 composite intent 怎么处理
...
- 是否应该先调用 skill 获取事实
- skill 的职责是返回事实，还是直接产出最终建议

推荐写法：

- “如果用户的问题同时包含天气查询 + 轻量决策建议，仍然应该先进入当前 skill 获取天气事实”
