...
### 2.1 多动作 skill 优先用 `action` 收口

如果一个 skill 的几个动作共享同一份业务对象，而且输入输出高度相关，优先做成一个 capability + 一个 skill，通过 `action` 分流。

当前仓库里的典型模式：

- `set_reminder`：`create / list / cancel`
- `manage_todo`：`create / list / complete`
- `manage_birthday`：`create / list`
- `capture_idea`：`create / list`

这样做的好处是：

- 路由语义集中，不会让系统在几个很像的 skill 之间摇摆
- `SKILL.md` 可以在同一份正文里写清不同 `action` 的参数准备规则
- `skill.json` 只需要统一映射一个 `action`
- capability handler 可以统一复用同一套 service / repository

prepare 阶段在这种模式下要做的事是：

- 先确定 `action`
- 再按该 `action` 准备所需槽位

不要把 `action` 漏掉，让 capability 自己猜。

如果 `action` 是修改、删除、完成、取消这类会影响既有记录的动作，优先准备稳定主键，例如：
...
- 同名记录可能不止一条
- 先 `list` 再按 id 操作，比按标题或正文模糊匹配更稳
- 这类动作一旦误命中，恢复成本明显高于创建动作

### 2.2 同 action 的多条记录优先用 `items`

如果用户一次想对多条同类记录执行同一个动作，优先准备：
...
这样 capability 可以逐条执行，并把成功和失败分别返回，不会因为其中一条失败就把整批都打掉。
...
### 3.8 相对日期示例不能写成“会被误复用的固定日期”

这一点在带时间解析的 skill 里非常重要，尤其是：

- `set_reminder`
- `get_weather`
- 任何会解析 `今天 / 明天 / 后天 / 周末 / 下周一` 的 skill

错误写法示例：

```md
- `明天上午9点` -> `2026-04-02 09:00`
```

这种写法会给 prepare 阶段一个非常危险的暗示：

- 它可能把 `2026-04-02` 当成一个可复用答案
- 而不是根据“当前运行日期”重新计算

更稳的写法应该是：

```md
- `明天上午9点` -> `<当前日期+1天 09:00>`
- `后天中午` -> `<当前日期+2天 12:00>`
- `今天12点` -> `<当前日期当天 12:00>`
```

如果确实需要给出绝对日期示例，必须显式写明前提：

```md
假设当前日期是 2026-04-01：
- `今天12点` -> `2026-04-01 12:00`
- `明天上午9点` -> `2026-04-02 09:00`
```
...
- 相对日期必须按当前运行时日期动态换算
- 不能直接复用文档示例中的绝对日期
- 当已经换算成具体日期后，向用户确认时应使用当前这一轮真实换算出的绝对日期
...
```json
{
  ...
  "retry_policy": {
    ...
    "backoff_min_ms": 200,
    "backoff_max_ms": 500,
    "retry_on": ["read_timeout", "connect_timeout", "http_502", "http_503", "http_504"]
  },
...
- `request_semantics.read_only=true`
  - 表示这个 skill 不修改外部系统状态
- `request_semantics.idempotent=true`
  - 表示同样输入执行多次，最终结果应等价
- `retry_policy.enabled=true`
  - 表示允许 runtime 对瞬时错误做快速重试
- `retry_policy.max_attempts=2`
  - 表示“首次请求 + 1 次重试”
- `retry_policy.retry_on`
  - 当前只建议包含瞬时错误
  - 例如 `read_timeout`、`connect_timeout`、`http_502`、`http_503`、`http_504`
...
如果 skill 是多动作模式，`request_input` 里通常还应显式映射：

```json
{
  "action": {
    "from": "prepared_payload.slots.action",
    "default": "create"
  }
}
```

这样 runtime 调 capability 时，不会把所有请求都默认为创建动作。
...
对于多动作 skill，还要额外检查一件事：

- 某个字段是不是只在某个 `action` 下才需要？

例如 `set_reminder`：

- `action=create` 时需要 `content`、`remind_at`
- `action=list` 时通常只需要 `action`，可选 `status`
- `action=cancel` 时需要 `reminder_id`，或者 `content + remind_at` 作为定位信息

这类字段在 `skill.json` 里通常不能一刀切写成全局 `required: true`，否则会把别的 `action` 卡死。
...
- 不要为了少数慢 skill 去全局抬高默认超时
- 交互型 skill 默认仍应保持较短超时，避免失败时尾延迟过长
- 当前更推荐“按 skill 单独覆写 `timeout_seconds`”，而不是统一从 20 秒提到 40/60 秒

当前实践建议：

- 普通交互型查询 skill：默认 20 秒左右
- 只读查询、但上游明显更慢的 skill：按 skill 单独提到 30 或 45 秒
- 真正长任务：优先走 `progress` + 更长硬上限，不要靠一个很大的统一超时硬撑

当前 runtime 的稳定字段仍只有：

- `timeout_seconds`

也就是说，`skill.json` 现在还不应直接写这些字段：
...
这些字段可以作为后续扩展方向，但在 runtime 没正式支持前，不要提前写进 `skill.json` 并假设会生效。

### 4.8 只有只读幂等 skill 才应该开启自动重试

runtime 现在会根据 `skill.json` 自动判断是否允许快速重试。
...
实践上，快速重试的优先级通常高于全局增大超时。

推荐策略：

- 只对只读、幂等 skill 开启
- 只重试瞬时错误
- 最多 1 次快速重试，也就是 `max_attempts=2`
- 退避保持很短，推荐 `200ms ~ 500ms`
...
- 这类有副作用的 skill 如果第一次其实已经成功，只是返回链路超时，自动重试就可能写出重复数据

`retry_on` 当前建议只包含瞬时失败：
...
- 如果 `retry_policy.enabled=true`，对应 capability 必须真的是只读且幂等
